...
if cylpor:
    ...
    p_me = np.loadtxt('../IO/porosity_meas_tot_volume.txt')
    p_im = np.loadtxt('../IO/porosity_image.txt')
    ...
    ax.set_xlabel('Air-filled porosity determined from \n water retention measurement')
    ax.set_ylabel('Air-filled porosity determined from \u03BCCT image')
    ...
    ax.set_position([0.13, 0.14, 0.83, 0.83])
...
if perc_switch:
    ...
    if True:
        ...
        thetas = np.loadtxt('../IO/wrc_meas_tot_volume_with_zero.txt')
        ...
        ax3.set_yscale('log')
        ax3.legend(handletextpad=0.2, ncol=2, columnspacing=0.5,borderaxespad=1)# loc=(0.53,0.05))
        ax3.set_xlabel('Air-filled porosity')# (m$^3$/m$^3$)')
        #ax3.set_ylabel('Pressure [kPa]')
        ax3.set_ylim(ylims)
        
        ax3.text(0.63, 0.46, 'Sample \nnumber', transform=ax3.transAxes)
